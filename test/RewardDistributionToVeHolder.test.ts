  ...
  for (let i = schedules.length - 1; i < schedules.length; i++) {
    describe('> Test scenario', function () {
      ...
      before(async function () {
        ...
        ;({ ethgasToken, tokenLock } = await fixture());
      ...
      describe('Staking', function () {
        ...
        it('ve staker claim staking reward', async function () {
          ...
          console.log("\nadvance half a week, fund 2000 gas token & claim reward")
          await moveToTime(tokenLock, BigNumber.from(latestTimestamp + 3600 * 24 * 3.5), 60);
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          ...
          console.log("\nadvance one more week, fund 4000 gas token & claim reward")
          await advanceOneWeek();
          // await advanceFourWeeks();
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          ...
          console.log(`\ncheckpoint total supply every 20 weeks, advance ${numberOfWeeksToUnlockStart} weeks to unlockStartTime & claim reward`)
          for (let i = 0; i <= numberOfWeeksToUnlockStart; i ++) {
            await advanceOneWeek();
            if (i % 20 === 0) {
              latestTimestamp = await getLatestBlockTimestamp()
              await feeDistributor.connect(contractAdminSigner).checkpoint_total_supply();
              console.log(new Date(latestTimestamp * 1000), "checkpoint_total_supply")
            }
          }
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          ...
          console.log("\nvesting user stake 3500 for 4 years")
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          ...
          console.log("\nadvance 2 more weeks, fund 3000 gas token & claim reward")
          await advanceOneWeek();
          await advanceOneWeek();
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          ...
          await feeDistributor.connect(contractAdminSigner).checkpoint_total_supply();
          await feeDistributor.connect(contractAdminSigner).checkpoint_token();
          ...
          console.log("\nadvance 3 more weeks, release vested token")
          await advanceOneWeek();
          await advanceOneWeek();
          await advanceOneWeek();
          ...
          console.log("\nadvance 1 more week, revoke vesting contract, fund 6000 gas token & claim reward")
          await advanceOneWeek();
          latestTimestamp = await getLatestBlockTimestamp()
          console.log(new Date(latestTimestamp * 1000))
          await fundEthgasToken(feeDistributor.address, parseTokenAmount("6000", "GWEI"))
          await feeDistributor.connect(contractAdminSigner).checkpoint_total_supply();
          await feeDistributor.connect(userSigners[2]).claim(userSigners[2].address)
          await feeDistributor.connect(userSigners[3]).claim(userSigners[3].address)
